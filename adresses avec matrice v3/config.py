...
def ecrireConfig(num_routeur, file, num_as, num_routeur_as, liensExt, lan): 
    ...
    file.write("!\nversion 15.2\nservice timestamps debug datetime msec\nservice timestamps log datetime msec  \n! \nhostname "
               ...
               # bgp
               + configBGP(num_routeur, num_as, typeRouteur, num_routeur_as, utilRouteur, liensExt, lan)
...
def adressage(num_routeur_as, interface, num_as, nomRouteur,  lan, liensExt):
    res = ""
    ...
    for n in range(len(obj_python["connexion"]["connections"])):
        if nomRouteur == obj_python["connexion"]["connections"][n]["id1"] and interface == obj_python["connexion"]["connections"][n]["interface1"]: 
            ...
            address = prefix_reseau_inter+str(liensExt[num_routeur1][num_routeur2])+ "."+\
    ...
    if(len(obj_python["AS"][num_as]["connections"])>0):
        for i in range(len(obj_python["AS"][num_as]["connections"])):
            if nomRouteur == obj_python["AS"][num_as]["connections"][i]["id1"] and interface == obj_python["AS"][num_as]["connections"][i]["interface1"]:
                ...
                address = prefix_reseau_intra + str(lan[num_routeur1][num_routeur2])+"."+str(num_routeur1)+" 255.255.255.0"
                return " \n"+" ip address "+address+"\n"+protocole+"\n"+vitesse
...
#ecrire bgp
def configBGP(num_routeur, num_as, typeRouteur, num_routeur_as, utilRouteur, liensExt, lan):
    ...
    if(typeRouteur=="bordure" ):
        ...
        if nombre_clients>0 and utilRouteur== "default":
            ...
            for i in range(len(obj_python["AS"][num_as]["routeurs"])):
                ...
                if ((routeur_voisin != num_routeur)and(obj_python["AS"][num_as]["routeurs"][i]["type"]=="bordure")):
                    if (obj_python["AS"][num_as]["routeurs"][i]["utilisation"] == "default"):
                    ...
                    if (obj_python["AS"][num_as]["routeurs"][i]["utilisation"] == "backup"):
                        for j in range(len(obj_python["AS"][num_as]["routeurs"][i]["destination"])):
                            if (obj_python["AS"][num_as]["routeurs"][i]["destination"][j][1:] == str(num_routeur)):
                                res= res + " neighbor "+prefixe+"."+str(lan[routeur_voisin][num_routeur])+"."+str(routeur_voisin)+" remote-as "+AS+"\n"+ " neighbor "+prefixe+"."+str(lan[routeur_voisin][num_routeur])+"."+str(routeur_voisin)+" update-source Loopback0\n"
        ...
        if nombre_clients > 0 and utilRouteur== "backup":   
            ...
            for j in range(len(obj_python["AS"][num_as]["routeurs"][num_routeur_as]["destination"])):
                routeur_voisin = int(obj_python["AS"][num_as]["routeurs"][num_routeur_as]["destination"][j][1:]) 
                res= res + " neighbor "+prefixe+"."+str(lan[routeur_voisin][num_routeur])+"."+str(routeur_voisin)+" remote-as "+AS+"\n"+ " neighbor "+prefixe+"."+str(lan[routeur_voisin][num_routeur])+"."+str(routeur_voisin)+" update-source Loopback0\n"
